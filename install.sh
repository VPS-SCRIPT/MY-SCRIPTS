...
cur_dir=$(pwd)
BASE_URL="https://x-ui.alireza0.dev"
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "${BASE_URL}/api/version" | grep -o '"version"[[:space:]]*:[[:space:]]*"[^"]*"' | head -1 | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}Failed to fetch x-ui version from ${BASE_URL}${plain}"
            exit 1
        fi
        echo -e "Got x-ui latest version: ${last_version}, beginning the installation..."
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch).tar.gz "${BASE_URL}/download/x-ui-linux-$(arch).tar.gz"
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Downloading x-ui failed, please be sure that your server can access ${BASE_URL}${plain}"
            exit 1
        fi
    else
        last_version=$1
        echo -e "Beginning to install x-ui ${last_version} (latest binary from ${BASE_URL})..."
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch).tar.gz "${BASE_URL}/download/x-ui-linux-$(arch).tar.gz"
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Downloading x-ui failed, please be sure that your server can access ${BASE_URL}${plain}"
            exit 1
    ...
    wget --no-check-certificate -O /usr/bin/x-ui "${BASE_URL}/x-ui.sh"
